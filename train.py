...
def main():
    ...
    print(attack_list_train)
    print(attack_list_val)
    ...
    for epoch in range(args.epoch):
        ...
        attack_list_train = [attack_list_train[0]]
        attack_list_val = [attack_list_val[0]]
        accs, acc, loss =0,0,0
        ...
        for step, (x, y) in enumerate(db):
            ...
            x_spt_clean, x_qry_clean = x[:spt_size], x[spt_size:batch_size]
            y_spt_clean, y_qry_clean = y[:spt_size], y[spt_size:batch_size]

            for i in range(len(attack_list_train)):
                x_spt_attack, x_qry_attack = x_spt_clean, x_qry_clean
                # attack the picture
                #print(attack_list_train[i][0],attack_list_train[i][1])
            ...
            accs, acc, loss = maml(x_spt, y_spt, x_qry, y_qry)
            update_lr = maml.update_lr
            meta_lr = maml.meta_lr
            print('step:', step, 'update_lr', update_lr, 'meta_lr', meta_lr,
                  '\ttraining acc:', acc, 'training loss:', loss)

        if epoch % 1 == 0:

            update_lr = maml.update_lr
            meta_lr = maml.meta_lr
            print('epoch:', epoch,'update_lr',update_lr,'meta_lr',meta_lr ,
                  '\ttraining acc:', acc, 'training loss:', loss)
            db_test = DataLoader(val_dataset, 1000, shuffle=True, num_workers=1, pin_memory=True)
            ...
            for step, (x, y) in enumerate(db_test):
                ...
                y = y.to(device)
                x_spt, y_spt, x_qry, y_qry = [], [], [], []
                x_spt_clean, x_qry_clean = x[:2], x[2:20]
                y_spt_clean, y_qry_clean = y[:2], y[2:20]
                for i in range(len(attack_list_val)):
                    x_spt_attack, x_qry_attack = x_spt_clean, x_qry_clean
                    # attack the picture
                    #print(attack_list_val[i][0], attack_list_val[i][1])
                    attack = get_attack('cifar-10', attack_list_val[i][0], attack_list_val[i][1], 10, 1, False)
                    attack = attack()
                    rand_target = torch.randint(0, 9, size=y_spt_clean.size(), device=device)
                    x_spt_attack = attack(maml.net, x_spt_clean, rand_target,
                                          avoid_target=True, scale_eps=False)
                ...
                x_spt, y_spt, x_qry, y_qry = torch.tensor(x_spt).to(device), torch.tensor(y_spt).to(device), \
                                             torch.tensor(x_qry).to(device), torch.tensor(y_qry).to(device)
                x_spt, y_spt, x_qry, y_qry = torch.squeeze(x_spt,0),torch.squeeze( y_spt,0),\
                                             torch.squeeze( x_qry,0), torch.squeeze(y_qry,0)
                accs, acc, loss = maml.finetunning(x_spt, y_spt, x_qry, y_qry)

            print( '\ttesting acc:', acc, 'testing loss:', loss)
