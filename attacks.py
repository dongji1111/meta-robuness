...
def get_eps_params(base_eps, resol):
    ...
    min_list = []
    for i in range(3):
        eps_list.append(torch.full((resol, resol), base_eps, device=device))
        min_list.append(torch.full((resol, resol), 0., device=device))
        max_list.append(torch.full((resol, resol), 255., device=device))
...
def get_imagenet_params(resol):
    ...
    for i in range(3):
        if torch.cuda.is_available():
            ...
            mean_list.append(torch.full((resol, resol), IMAGENET_MEAN[i], device='cpu'))
            std_list.append(torch.full((resol, resol), IMAGENET_STD[i], device='cpu'))
